...
import { Award, Search, Filter, ArrowLeft, Calendar, ExternalLink, Download } from 'lucide-react'
import Image from 'next/image'
import Link from 'next/link'
import { CertificationData, allCertifications, categories, years } from '@/lib/certificationData'
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between text-xs font-mono">
            ...
            <span className="text-cyan-400 font-medium font-mono text-xs truncate">{certificate.credentialId}</span>
          </div>
          <div className="flex items-center justify-between text-xs font-mono">
...
export default function AllCertificatesShowcase() {
  const [filteredCertificates, setFilteredCertificates] = useState(allCertifications)
  ...
  useEffect(() => {
    ...
    if (selectedCategory !== 'All') {
      filtered = filtered.filter((cert: CertificationData) => cert.category === selectedCategory)
    }

    // Filter by year
    if (selectedYear !== 'All') {
      filtered = filtered.filter((cert: CertificationData) => cert.issueDate === selectedYear)
    }

    // Filter by search term
    if (searchTerm) {
      filtered = filtered.filter((cert: CertificationData) =>
        ...
        <div className="container-max">
          ...
          >
            ...
            <p className="text-gray-300 font-mono text-lg max-w-3xl mx-auto leading-relaxed">
              Complete portfolio of {allCertifications.length} professional certifications spanning cloud computing,
          ...
          >
            <p className="text-gray-400 font-mono text-sm">
              Showing {filteredCertificates.length} of {allCertifications.length} certifications
            </p>
          </motion.div>

          {/* Certificates Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredCertificates.map((certificate: CertificationData, index: number) => (
              <CertificateCard
                key={certificate.id}
